...
export const BuilderWorkspace: React.FC<BuilderWorkspaceProps> = ({ initialPrompt, apiKey, goBack, openSettings }) => {
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-white/95 backdrop-blur-sm text-gray-700 font-sans overflow-hidden relative">
      ...
      {/* --- HEADER --- */}
      <header className="h-14 md:h-16 border-b border-gray-200 bg-white/90 backdrop-blur-md flex items-center justify-between px-3 md:px-4 z-20 shadow-sm relative shrink-0">
        <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
          ...
          <div className="flex flex-col overflow-hidden">
            <h2 className="text-[10px] md:text-xs font-bold text-gray-600 tracking-wide uppercase truncate" title={history[historyIndex]?.prompt}>
              {history[historyIndex]?.prompt || "Initializing Workspace..."}
            </h2>
            <div className="flex items-center gap-2 mt-0.5">
              ...
              <span className="text-[9px] md:text-[10px] font-mono text-gray-700">{isBuilding ? 'SYNTHESIZING...' : 'READY'}</span>
            </div>
          </div>
        </div>

        {/* Central History Controls - Desktop only */}
        <div className="hidden lg:flex items-center gap-2 bg-gray-100 p-1 rounded-lg border border-gray-200 shadow-sm">
          <Tooltip text="Undo">
            <Button variant="icon" onClick={handleUndo} disabled={historyIndex <= 0}><Undo2 size={16}/></Button>
          </Tooltip>
          <span className="text-xs font-mono text-gray-600 min-w-[60px] text-center select-none">
            v{historyIndex + 1} / {history.length}
          </span>
          <Tooltip text="Redo">
            <Button variant="icon" onClick={handleRedo} disabled={historyIndex >= history.length - 1}><Redo2 size={16}/></Button>
          </Tooltip>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          <Button 
            ...
            className={`!py-2 !px-3 md:!px-4 text-xs ${isBuilding ? 'opacity-70' : ''}`}
      ...
      <div className="flex-1 flex overflow-hidden z-10 relative">
          ...
          <div className="flex border-b border-gray-200 bg-white/80 backdrop-blur-sm overflow-x-auto hide-scrollbar">
            {['preview', 'code', 'logs'].map((tab) => (
              <button
                ...
                className={`
                  flex-1 md:flex-none px-4 md:px-6 py-3 text-xs font-mono font-bold flex items-center justify-center gap-2 border-r border-gray-200 transition-colors min-w-[100px]
                  ${activeTab === tab ? 'text-gray-900 bg-white border-t-2 border-t-orange-500' : 'text-gray-500 hover:bg-gray-50'}
                `}
              >
                {tab === 'preview' && <PlayCircle size={14} />}
               ...
               <div className="absolute inset-0 flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
                 ...
                 <div className="flex items-center justify-between px-4 py-3 bg-gray-800/50 border-b border-gray-700/50 backdrop-blur-sm">
                   <div className="flex items-center gap-3">
                     ...
                     <span className="text-xs font-mono text-gray-400 font-semibold">SOURCE CODE</span>
                   </div>
                   <Button variant="dark" icon={Copy} onClick={handleCopyCode} className="!py-1.5 !px-3 !text-xs">Copy</Button>
                 </div>
                 ...
                 {/* Terminal Footer */}
                 <div className="px-4 py-2 bg-gray-800/50 border-t border-gray-700/50 backdrop-blur-sm">
                   <div className="flex items-center justify-between text-[10px] font-mono text-gray-500">
                     <span>LENGTH: {currentCode.length} CHARS</span>
        ...
        {/* RIGHT PANE: HISTORY (Collapsible on Mobile) */}
        <div className="hidden xl:flex w-[320px] flex-col border-l border-gray-200 bg-white/90 backdrop-blur-md">
           <div className="p-4 border-b border-gray-200 font-mono text-xs font-bold text-gray-600 flex items-center gap-2 select-none">
            <History size={14} /> TIMELINE
           </div>
           <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-3">
              {history.length === 0 && <div className="text-center text-gray-500 text-xs py-10">Timeline empty</div>}
              {history.map((entry, idx) => (
                <div 
                  key={idx}
                  onClick={() => { setHistoryIndex(idx); setIframeKey(k => k+1); }}
                  className={`group relative p-3 rounded-lg cursor-pointer border transition-all duration-200 ${idx === historyIndex ? 'bg-orange-50 border-orange-500 shadow-[0_0_15px_rgba(234,88,12,0.15)]' : 'bg-white border-gray-200 hover:border-gray-300 hover:shadow-sm'}`}
                >
                  <div className="flex justify-between items-center mb-2">
                    <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded font-bold tracking-wider ${idx === 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700'}`}>
                      {idx === 0 ? 'GENESIS' : 'ITERATION'}
                    </span>
                    <span className="text-[10px] text-gray-500 font-mono">{new Date(entry.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                  </div>
                  <p className="text-xs text-gray-700 line-clamp-2 leading-relaxed font-medium group-hover:text-gray-900 transition-colors">{entry.prompt}</p>
                  
                  {idx === historyIndex && (
                     <div className="absolute -right-1 top-1/2 -translate-y-1/2 w-2 h-8 bg-orange-500 rounded-l"></div>
                  )}
                </div>
              ))}
           </div>
           ...
           <div className="p-4 border-t border-gray-200 bg-gray-50">
             <div className="text-[10px] text-gray-600 text-center">
               <span className="text-orange-500 font-bold">PRO TIP:</span> Use arrows keys to time travel.
             </div>
           </div>
        </div>
